...
var currentFinalStep = 0;
function toggleFinalStep() {
    var steps = [ "final-step-1", "final-step-2", "final-step-3", "-"];

    steps.forEach( function( c ){ visualization.classList.remove( c ); });

    currentFinalStep = currentFinalStep + 1;
    if (currentFinalStep >= steps.length) currentFinalStep = 0;
    visualization.classList.add( steps[ currentFinalStep ]);
}

document.addEventListener("keyup", function( event ){

    switch ( String.fromCharCode( event.keyCode ) ) {
        case "Q": visualization.classList.remove( "expanded" );
                  visualization.classList.remove( "final" );
                  break;
        case "W": visualization.classList.add( "expanded" );
                  visualization.classList.remove( "final" );
                  break;
        ...
        case "E": visualization.classList.remove( "expanded" );
                  ...
                  break;
        case "D": toggleFinalStep();
                  break;
        case "Z": visualization.classList.toggle( "zoomed" );
                  break;
    }
